...
class Tokenizador:
    def __init__(self, codigo):
        ...
        self.palavras_chave = self.palavras_reservadas + self.operadores_aritmeticos + self.operadores_logicos + self.simbolos_especiais
    ...
    def verifica_valor(self):
        ...
        for i in range(len(self.matriz_tokens)):
            if (i+2) < len(self.matriz_tokens):
                tipo_token = self.matriz_tokens[i+2][1]
                if (self.matriz_tokens[i][1] == "IDENT" and
                    self.matriz_tokens[i+1][1] == ":=" and
                    tipo_token == "LITERAL_INT" or 
                    tipo_token == "LITERAL_STRING" or
                    tipo_token == "BOOLEAN"
                ):
    ...
    def tokenizar(self):
        ...
        for i, char in enumerate(self.codigo):
            ...
            if (
                next_char == " " or
                next_char in self.palavras_chave or
                lexema in self.palavras_chave or
                ...
                if lexema != '':
                    ...
                    if lexema == "\n":
                        ...
                        if re.match(self.string_constante, lexema) != None:
                            ...
                            second_item = "BOOLEAN" if lexema in ["true", "false"] else lexema
                            ...
                            curr_lex in self.esp_simbolos or 
                            lexema in self.operadores_aritmeticos or 
                            lexema in self.operadores_logicos or 
                            lexema in self.simbolos_especiais
                            ):
                                second_item = lexema
                        else:
                            erro = True
        ...
        if erro == True:
            e = ErrorMessage(29, self.erro_elemento[1], self.erro_elemento[2], self.erro_elemento[0])
            e.erro_mensagem_model()

        # Conversão dos valores do dicionário de ids para uma lista
        ids_list = [self.ids.get(self.matriz_tokens[i][0]) for i in range(len(self.matriz_tokens))]

        valores = self.verifica_valor()
        valores_list = [valores.get(self.matriz_tokens[i][0]) for i in range(len(self.matriz_tokens))]

        self.matriz_tokens = [self.matriz_tokens[i] + [ids_list[i], valores_list[i]] for i in range(len(self.matriz_tokens))]
